...
We get this command `nc challenge.ctf.games 30877`.

For exemple with `ls` :
![ls](exemple_ls.png)

So we have an exotic shell.


But we can try to redirect the standard output somwhere else such as the `error` or `input` output.
And :
```
ls
...
ls 1>&0
 ...
 ""#                  mmm             mmm   mmmm 
   ...
   #    #   "           #     """mm  ##    #  m #
   #     """m           #     mmm"" #  #m# #    #
   "mm  "mmm"         mm#mm  "      "#mm#m  #mm# 
                                                 
                                                 
flag.png
```
And to have a better display we can try :
```
bash
...
It is an image so we can get it by base64 and see it on our pc :
```
base64 flag.png 1>&0
base64 flag.png 1>&0
...
And finally :
```bash
cat flag.base64 | base64 -d
```

And just need to open the picture and note the flag : `flag{flushed_down_the_toilet_but_rescued_again}`